...
import { UserData, User } from '../../../_core/backend/interfaces/users';
import { EMAIL_PATTERN, NUMBERS_PATTERN } from '../../../_auth/components';
  ...
  save() {
    ...
    if (this.mode === UserFormMode.EDIT_SELF) {
      this.usersService.updateCurrent(user).subscribe((result: any) => {
          this.tokenService.set(new NbAuthOAuth2JWTToken(result, 'oauth', new Date()));
